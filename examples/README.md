...
| `hono-map/server.ts` | `tinytsx build hono-map/server.ts --output server --release` | Non-escaping 16-entry request-local Map with primitive keys and values |
| `hono-stytch-todo/server.ts` | `tinytsx build hono-stytch-todo/server.ts --binding TODOS=sqlite-kv::memory: --alias hono="$TINYTSX_HOME/vendor/hono/src/index.ts" --alias hono/cors="$TINYTSX_HOME/vendor/hono/src/middleware/cors/index.ts" --api hono=hono-stytch-todo/hono-api.d.ts --api hono/cors=hono-stytch-todo/cors-api.d.ts --api @hono/stytch-auth=hono-stytch-todo/stytch-api.d.ts --output server --release` | Three byte-identical pinned upstream backend modules with credential-free session and SQLite KV adapters |
...
| `hono-actors/supervision.ts` | `tinytsx build hono-actors/supervision.ts --output server --release` | Static one-for-one root supervisor with a shared bounded restart window |
