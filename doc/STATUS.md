...
- The minimized runtime regression is green, the original default-worker
  ...
  pass 22/22 and 70/70 under strict Clippy. The controlled P4 rerun at clean
  source `932743e` retains three 15-second samples at concurrency 8/64 for the
  basic, actor, SQLite, and nested-profile routes. All 48 samples pass, every
  TinyTSX process returns from 68 descriptors to four, and concurrency-64
  throughput is 0.39–0.63x Bun with 12.576–16.008 ms p99. Compared with the
  earlier basic control, the fix reduces throughput and raises aggregate CPU;
  it is a bounded correctness/stability tradeoff. Exact-source Apple and native
  Linux release verification remains required at the final documentation
  commit.
...
### Prior Apple release verification (2026-07-18)
...
- A later complete clean Apple release verification at `947cd8b` also passes;
  its archive checksum is
  `5b015cad1691e5dd2fe0a107ccc866bdd43e6d421f7242a6c63e3eb81aeb38a9`.
  Pressure-aware scheduler and evidence commits landed afterward, so neither
  historical archive attests the current source. The next candidate must rerun
  Apple and native Linux from the same final clean commit.
