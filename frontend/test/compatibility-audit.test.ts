...
test("executes the unchanged TodoService private set and delete fields", () => {
  const entry = write("stytch-todo-service-delete.ts", `
    import type {} from "@hono/stytch-auth";
    import {Hono} from "hono";
    import {todoService} from "stytch-todo-service";
    ...
    const environment = ({
      TODOS: {
        get: async () => [
          {id: "todo-3", text: "third", completed: true},
    ...
    const app = new Hono();
    app.delete("/todos", async context => {
      const todos = await todoService(environment, "user-1").delete("todo-2");
      return context.json({todos});
    });
    export default app;
  `);
  ...
  assert.equal(hir.handlers[0]?.method, "DELETE");
  assert.ok(hir.staticStrings.some(value => value.value === JSON.stringify({todos: [
    {id: "todo-1", text: "first", completed: false},
    {id: "todo-3", text: "third", completed: true},
  ]})));
...
test("executes the unchanged TodoService find and record mutation", () => {
  const entry = write("stytch-todo-service-complete.ts", `
    import type {} from "@hono/stytch-auth";
    import {Hono} from "hono";
    import {todoService} from "stytch-todo-service";
    ...
    const app = new Hono();
    app.post("/todos/complete", async context => {
      const todos = await todoService(environment, "user-1").markCompleted("todo-2");
      return context.json({todos});
    });
    export default app;
  `);
