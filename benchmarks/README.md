...
- `hono-json-body` posts one fixed bounded JSON object through a shared Hono
  route and returns selected string, number, boolean, and null fields with their
  types and escaping preserved;
...
- `hono-route-param` decodes one trailing optional route parameter and returns
  it through a bounded JSON response; and
- `hono-file-read` reads the pinned 21-byte serve-static asset on every request
  through `tinytsx:fs` or `Bun.file` and returns it through the same Hono route;
  and
- `hono-large-file` repeats that warm-cache shape with the pinned 22,173-byte
  Hono context source and a 32 KiB TinyTSX file bound; and
...
npm run benchmark:hono-file-read
...
faults, thread count, open file descriptors, and RSS. RSS, threads, and open
descriptors are sampled every 20 ms; start/peak/end descriptor counts are
retained while cumulative counters are captured at the measurement boundaries.
These are whole-server process counters, not per-route profiles, and the load
generator runs on the same machine.
...
```bash
python3 benchmarks/scripts/run_static.py \
  --workload hono-basic --duration 15 --runs 3 --startup-runs 5 \
  --concurrency 8,64 --workers 8 --keep-alive \
  --output-prefix benchmarks/results/local-sustained-hono-basic
python3 benchmarks/scripts/run_static.py \
  --workload hono-json-body --duration 15 --runs 3 --startup-runs 5 \
  --concurrency 8,64 --workers 8 --keep-alive \
  --output-prefix benchmarks/results/local-sustained-hono-json-body
python3 benchmarks/scripts/run_static.py \
  --workload hono-json-compact --duration 15 --runs 3 --startup-runs 5 \
  --concurrency 8,64 --workers 8 --keep-alive \
  --output-prefix benchmarks/results/local-sustained-hono-json-compact
python3 benchmarks/scripts/run_static.py \
  --workload hono-json-pretty --duration 15 --runs 3 --startup-runs 5 \
  --concurrency 8,64 --workers 8 --keep-alive \
  --output-prefix benchmarks/results/local-sustained-hono-json-pretty
python3 benchmarks/scripts/run_static.py \
...
python3 benchmarks/scripts/run_static.py \
  --workload hono-file-read --duration 15 --runs 3 --startup-runs 5 \
  --concurrency 8,64 --workers 8 --keep-alive \
  --output-prefix benchmarks/results/local-sustained-hono-file-read
python3 benchmarks/scripts/run_static.py \
  --workload hono-large-file --duration 15 --runs 3 --startup-runs 5 \
  --concurrency 8,64 --workers 8 --keep-alive \
  --output-prefix benchmarks/results/local-sustained-hono-large-file
python3 benchmarks/scripts/run_static.py \
  --workload hono-stream-text --duration 15 --runs 3 --startup-runs 5 \
  --concurrency 8,64 --workers 8 --keep-alive \
  --output-prefix benchmarks/results/local-sustained-hono-stream-text
python3 benchmarks/scripts/run_static.py \
  --workload hono-actor --duration 15 --runs 3 --startup-runs 5 \
  --concurrency 8,64 --workers 8 --keep-alive \
